...
function ss_composeSuggestUri(qVal, suggestForm) {
  ...
  //DBS-Kevin Modified - BEGIN
  //var uri = '/responder.aspx';
  var uri = '';
  //DBS-Kevin Modified - END
...
function ss_suggest(qVal) {
  ...
  var url = ss_gsa_host + ss_gsa_param + '&q=' + encodeURIComponent(qVal);
  //DBS-Kevin Modified - END
...
function ss_handleAllKey(e) {
  ...
  switch (kid) {
    ...
    case 16:  // "shift-tab".
      ss_qbackup = null;
      ss_dismissed = true;
      var qry = $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val();
      if (!ss_processed(qry)) {
...
function ss_handleKey(e) {
  ...
  var qnow = (!ss_qbackup) ? $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val() : ss_qbackup;
  var sum = 0;
  ...
  switch (kid) {
    case 40:  // "key down".
      ...
      if (ss_processed(qnow)) {
        ...
        if (sum > 0) {
          ...
          if (suggestion.q) {
            $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val(suggestion.q);
          } else {
            $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val(ss_qbackup);
          }
        }
      } else {
    ...
    default:
      ss_dismissed = false;
      if ($('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val() == ss_qshown) {
        ...
        ss_waiting++;
        if (ss_allow_debug && ss_debug && ss_debug.getDebugMode()) {
          ss_debug.addWaitDebugLine($('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val(), 'queue', ss_wait_millisec);
        }
        ss_key_handling_queue = setTimeout(
            'ss_handleQuery("' + ss_escape($('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val()) + '", ' +
...
function ss_sf() {
  ...
  $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').focus();
  ss_dismissed = false;
}
...
function ss_show(qry) {
  var currentQry = $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val();
  if (currentQry != qry) {
  ...
  if (cnt > 0) {
    ...
    clsTxt.onclick = function() {
      ...
      ss_clear();  // This will always turn off ss_dismiss after bring search
                   // box into focus.
      var query = $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val();
...
function ss_handleMouseM() {
  ...
  for (var ri = 0; ri < rows.length - 1; ri++) {
    if (rows[ri] == this && rows[ri].className != SS_ROW_SELECTED_CLASS) {
      ...
      // index.
      if (!ss_qbackup) {
        ss_qbackup = $('input[type="text"][gsa-searchbox-id="'+selectedGsaSearchBox+'"]').val();
      }
      ss_loc = ri;
      // Find out what type of suggestion it is.
      var suggestion = ss_locateSuggestion(ss_qbackup, ss_loc);
  ...
  //DBS-Kevin Removed - BEGIN
  //ss_sf();
  //DBS-Kevin Removed - END
  return true;
}
