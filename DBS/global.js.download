   ...
   $(window).load(function() {
       setTimeout(function() {
           if(document.location.hash !="" ) {
               var atab=document.location.hash;$(atab).parent().trigger("click");
               var elmt=$(atab)
               var ht=0
               /*
                var ext//=40
                if($(window).width()>767) {
                if($(window).width()<1281) ext=elmt.closest(".tabbed-content.page-module").find(".section-title").outerHeight()
                else ext=elmt.closest(".tabbed-content.page-module").find(".section-title").outerHeight()+20
                }
                */

               var ext=0
               if($(window).width()>767) {
                   if($(window).width()<1281) ext=0
                   else ext=20
               }
               ...
               /*var ext=40
                if($(window).width()>767) {
                if($(window).width()<1281) ext=elmt.closest(".tabbed-content.page-module").find(".section-title").outerHeight()
                else ext=elmt.closest(".tabbed-content.page-module").find(".section-title").outerHeight()+10
                } */

               ht=elmt.closest(".tabbed-content.page-module").find(".section-title").outerHeight()+ext ;
               elmt.closest(".tabbed-content.page-module").prevAll().each(function(){
                   ht+=$(this).outerHeight();
               })
               var elmt1=elmt.closest(".tabbed-content.page-module").parent()
               if(elmt1.length>0){
               while(!elmt1.hasClass("main-container")){
                   elmt1.prevAll().each(function(){
                       ht+=$(this).outerHeight();
                   })
                   elmt1=elmt1.parent()
               }
               if($(window).width()<4281){
                   $('html, body').animate({
                       scrollTop: ht
                   }, 10);
               }
	 ...
	 function filltabs(elementt)
        ...
        $(".tabbed-content.product-info .tabbed-nav.tabs li").click(function(eventObject) {
		  ...
		  $('.tabbed-content:not(.product-info)').each(function(ii, elementt) {
		   ...
		   /*
		   var grp=0
			$(".tabbed-content.page-module:not(.product-info)").each(function(eventObject) {
			grp++
			$(this).find(".tabbed-nav.tabs li").each(function(eventObject) {
				var oldid=$(this).find("a").attr("id")
				if(oldid!=undefined){
				var txt1=oldid.replace(/[<>^a-zA-Z 0-9]+/g,'')
				var newid=oldid.replace(txt1,'').trim();
				$(this).find("a").attr("id","g"+(grp)+newid);
				}
			})
			}) */
		 ...
		 //Added for MR 1602
			 /*
		if($(this).find("a").attr("id")!=undefined) document.location.hash=$(this).find("a").attr("id")
                                else document.location.hash="" */
			//End for MR 1602
             var scrollTab=$(this).parent().get(0);
			...
			/* if($(this).closest(".page-module").find(".active a").attr("id")!=undefined) document.location.hash=$(this).closest(".page-module").find(".active a").attr("id")
            else document.location.hash=""
				var elmt=$(this)
				var ht=elmt.closest(".tabbed-content.page-module").find(".section-title").height()	
				elmt.closest(".tabbed-content.page-module").prevAll().each(function(){
				ht+=$(this).height()
				})
				ht+=$(".breadcrumb-area").height()+$("#header-mobile").height()+70
				//if($(this).find("a").attr("id")!=undefined && elmt.find("a").attr("id").indexOf("g2")>-1) ht+=20
				if($(this).closest(".page-module").find(".active a").attr("id")!=undefined && $(this).closest(".page-module").find(".active a").attr("id").indexOf("g2")>-1)ht+=20
				if($(window).width()>600) ht=elmt.offset().top
				if(document.location.hash!=""){
        ...
        $(".overview_overlap .tabbed-content").each(function(){
...
/* AICS QR END */},
  disclaimerCheck:function () {//updated for disclaimer check - Hariharan D - 19 Sep
            ...
            if ($(".subscribe-block").length == 0) {
                ...
                /* Changed for YDBS Email Validation */
                var emailTest = /^([a-zA-Z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,14}|[0-9]{1,14})(\]?)$/;
        ...
        //$.loginHomeMenu(40,330,dataIntervalGIT);
                        if(typeof window.matchMedia != "undefined"){
                                        ...
                                        if(mql.matches || mqlTab.matches){
                                           $.loginHomeMenu(60,330,dataIntervalGIT);
                                        }else{
                                                                        $.loginHomeMenu(40,330,dataIntervalGIT);
                                        }
                        }
...
function animateEle(e) {    
	...
	//								startTimeout();
		...
		HeroBannerMobile=function() {
			...
			pointer = 0;
			/* placing the second item text for the carousel tab display area - match with desktop
...
$.ajax({
    ...
    success: function (xml) { 
        ...
        var cardCarouselTemplate = 
			[
			'<div class="span3 positionrelative" data-category="{{categories}}">', '<div class="card_tick"><img src="/iwov-resources/images/tick_bullet_red.png" /></div>',
			' <img id="{{id}}" data-applyLink="{{applyLink}}" data-cardLink="{{cardLink}}" data-tagline="{{tagline}}" data-minIncome="{{minIncome}}" data-foreignIncome="{{foreignIncome}}" data-waiver="{{waiver}}" data-rewardsHeading="{{RewardsHeading}}" data-item="{{item}}" data-RewardsItem="{{RewardsItem}}" data-type="{{type}}"  data-dbsshopper="{{dbsshopper}}" data-shoppingOthers="{{shoppingOthers}}"  data-spc="{{spc}}" data-esso="{{esso}}" data-motoringOthers="{{motoringOthers}}" data-indulge="{{indulge}}" data-amex="{{amex}}"  data-gv="{{gv}}" data-shaw="{{shaw}}" data-cathay="{{cathay}}"  data-privileges="{{privileges}}" data-excl="{{excl}}" data-dbshealthcare="{{dbshealthcare}}" data-rewardpoints="{{rewardpoints}}" data-additionalpoints="{{additionalpoints}}" src="/iwov-resources/images/cards/{{imgurl}}" data-cardname="{{cardName}}" data-imgurl= "/iwov-resources/images/cards/{{imgSrc}}" class="cursor-hand cardsBorder">',
			' <div class="addCard" >',
			  '    <div class="h3-mimic">{{name}}</div>',  
			' </div>',  
			'</div>'
...
var selected_card = [];
$( "#comparecards" ).on("click", "img", function() { 
...
function getCardDetail(clickedObj,callFrm){ 
	...
	function displayCompareCards(clickedObj){ 
		var cardId = clickedObj.attr("id"),
		...
		var imgPath ='/iwov-resources/images/';
...
/* IN1896113 */
if($("iframe").length>0 && $(window).width()<=900){
 if($(window).width() <768){
  $("iframe").css("width",$(".main-container .span12").width()-20)
 }else{
  $("iframe").css("width",($(".main-container .span12").width()/3*2)-20)
 }
}
$(window).on("orientationchange", function(event) {
 setTimeout(function () {
 if($("iframe").length>0 && $(window).width()<=1025){
  if($(window).width() <768){
   $("iframe").css("width",$(".main-container .span12").width()-20)
  }else{
   $("iframe").css("width",($(".main-container .span12").width()/3*2)-20)
...
$('.megamenu').mouseover(function(){$('.loansMenu').blur();$('.loansSubMenu').blur()});	
	...
	if (targetTab != "") {
		...
		if (!(flag)) {
            ...
            $('.tabbed-nav li[data-index="0"]').addClass('active');
...
$(window).load(function() {
      ...
      //IN998130 Issue fix.
		...
		/*
		  ...
		  if(atab!=""){
			  setTimeout(function () {
				  $(atab).parent().trigger("click")
				  },10)
		    $('ul.tabbed-nav.tabs.hidden-phone li').each(function(i, element) {
		                  if ($(element).hasClass('active')) {
		                      $(element).removeClass('active'); 
		                  }
		              });
		  $(atab).parent().addClass("active");
		  var index=$(atab).parent().attr('data-index');   
		  //console.log(index); 
		  $('.tabcontent').hide();
		  var contdiv=$(atab).parent().parent().parent();
		//  console.log(contdiv);
		  $(contdiv).find(".tabcontent").eq(index).show();
		  }
		  */
});
